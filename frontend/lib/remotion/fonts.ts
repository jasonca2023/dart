import { loadFont as loadFraunces } from "@remotion/google-fonts/Fraunces";
import { loadFont as loadNewsreader } from "@remotion/google-fonts/Newsreader";
import { loadFont as loadBricolage } from "@remotion/google-fonts/BricolageGrotesque";
import { loadFont as loadBigShoulders } from "@remotion/google-fonts/BigShoulders";
import { loadFont as loadJetBrains } from "@remotion/google-fonts/JetBrainsMono";
import type { FontKey } from "../adSpec";

// Real, characterful faces (Hallmark allowlist) — one per mood register so each
// mood reads differently, not just in colour:
//   serif     -> Fraunces (luxe: editorial high-contrast)
//   softserif -> Newsreader (calm: gentle reading serif)
//   grotesque -> Bricolage Grotesque (playful/bold: display)
//   condensed -> Big Shoulders (energetic: industrial/athletic condensed)
//   mono      -> JetBrains Mono (techy: engineering)
const serif = loadFraunces("normal", { weights: ["400", "600", "900"], subsets: ["latin"] });
const softserif = loadNewsreader("normal", { weights: ["400", "600", "800"], subsets: ["latin"] });
const grotesque = loadBricolage("normal", { weights: ["600", "800"], subsets: ["latin"] });
const condensed = loadBigShoulders("normal", { weights: ["600", "800"], subsets: ["latin"] });
const mono = loadJetBrains("normal", { weights: ["500", "700"], subsets: ["latin"] });

const FALLBACK: Record<FontKey, string> = {
  serif: 'Georgia, "Times New Roman", serif',
  softserif: 'Georgia, "Times New Roman", serif',
  grotesque: 'system-ui, -apple-system, "Segoe UI", Roboto, sans-serif',
  condensed: '"Arial Narrow", system-ui, sans-serif',
  mono: 'ui-monospace, Menlo, monospace',
};

export const FONT_FAMILY: Record<FontKey, string> = {
  serif: `"${serif.fontFamily}", ${FALLBACK.serif}`,
  softserif: `"${softserif.fontFamily}", ${FALLBACK.softserif}`,
  grotesque: `"${grotesque.fontFamily}", ${FALLBACK.grotesque}`,
  condensed: `"${condensed.fontFamily}", ${FALLBACK.condensed}`,
  mono: `"${mono.fontFamily}", ${FALLBACK.mono}`,
};

// Resolves when the web fonts have loaded. The in-browser renderer awaits this
// before rasterizing so the exported video uses the real faces, not the fallback.
export const fontsReady: Promise<unknown> = Promise.all([
  serif.waitUntilDone(),
  softserif.waitUntilDone(),
  grotesque.waitUntilDone(),
  condensed.waitUntilDone(),
  mono.waitUntilDone(),
]);
